...
void CInput::Update()
{
	for (int64_t i = 0; i < sizeof(keys); i++)
	{
		prevKeys[i] = keys[i];
	}
...
bool CInput::IsKeyDown(int64_t _key)
{
	return KEYDOWN(keys, _key);
}

bool CInput::IsKeyPressed(int64_t _key)
{
	return (
...
bool CInput::IsKeyRelease(int64_t _key)
{
	return (
		!KEYDOWN(keys, _key) && KEYDOWN(prevKeys, _key)
	);
...
bool CInput::IsKeyHold(int64_t _key)
{
	return (
		KEYDOWN(keys, _key) && KEYDOWN(prevKeys, _key)
		);
