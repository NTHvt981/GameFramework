...
CBoss::CBoss()
{
	...
	movementMatrix.push_back(std::pair<Vector, float>(Vector(BOSS_MAX_RANGE, 0), BOSS_MOVE_NORMAL));
	movementMatrix.push_back(std::pair<Vector, float>(Vector(BOSS_MED_RANGE, BOSS_MIN_RANGE), BOSS_MOVE_SLOW));
	movementMatrix.push_back(std::pair<Vector, float>(Vector(0, 0), BOSS_MOVE_SLOW));

	movementMatrix.push_back(std::pair<Vector, float>(Vector(-BOSS_MAX_RANGE, 0), BOSS_MOVE_NORMAL));
	movementMatrix.push_back(std::pair<Vector, float>(Vector(-BOSS_MED_RANGE, BOSS_MIN_RANGE), BOSS_MOVE_SLOW));
	movementMatrix.push_back(std::pair<Vector, float>(Vector(0, 0), BOSS_MOVE_SLOW));

	movementMatrix.push_back(std::pair<Vector, float>(Vector(0, -BOSS_MAX_RANGE), BOSS_MOVE_SLOW));
	movementMatrix.push_back(std::pair<Vector, float>(Vector(0, -BOSS_MED_RANGE), BOSS_MOVE_NORMAL));
	movementMatrix.push_back(std::pair<Vector, float>(Vector(0, BOSS_MAX_RANGE), BOSS_MOVE_FAST));
	movementMatrix.push_back(std::pair<Vector, float>(Vector(BOSS_MED_RANGE, BOSS_MAX_RANGE), BOSS_MOVE_SLOW));
	movementMatrix.push_back(std::pair<Vector, float>(Vector(0, 0), BOSS_MOVE_SLOW));
...
void CBoss::GetAllCollisionBoxes(std::list<LPCollisionBox>& listCo)
{
	listCo.clear();
...
void CBoss::InflictDamage(int64_t dam)
{
...
void CBoss::HandleMovement(DWORD dt)
{
	...
	int64_t _signX = _x >= 0 ? 1 : -1;
	int64_t _signY = _y >= 0 ? 1 : -1;
...
void CBoss::UpdateHand(DWORD dt, CBossHand* hand, int64_t& index)
{
	hand->Update(dt);
