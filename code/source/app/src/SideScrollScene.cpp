...
{
	int64_t tile_count, tile_width, tile_height, tile_count_width, tile_count_height;
	std::vector<int64_t> solid_tiles;
	std::vector<int64_t> anti_player_tiles;

	std::wstring tileSetDir;

	std::string tileMatrixDir;
...
void CSideScrollScene::LoadWalls(
	std::vector<std::vector<int64_t>> matrix, std::vector<int64_t> solid_tiles, int64_t tile_width)
{
	std::vector<Box<float>> solid_boxes;
	SetWallPosition(solid_boxes, matrix, solid_tiles, tile_width);
	for (const Box<float>& box: solid_boxes)
	{
		AddEntity(
...
void CSideScrollScene::LoadAntiPlayerZones(
	std::vector<std::vector<int64_t>> matrix, std::vector<int64_t> anti_player_tiles, int64_t tile_width)
{
	std::vector<Box<float>> solid_boxes;
	SetWallPosition(solid_boxes, matrix, anti_player_tiles, tile_width);
	for (Box<float>& box: solid_boxes)
	{
		AddEntity(
...
void CSideScrollScene::LoadPortals()
{
	...
	{
		int64_t id = ls[i][0];
		int64_t areaId = id / 10;
		int64_t reverseId = (id % 10) * 10 + (id / 10);

		int64_t l = ls[i][1] * 32;
		int64_t r = ls[i][3] * 32 + 32;

		int64_t t = ls[i][2] * 32;
		int64_t b = ls[i][4] * 32 + 32;
...
{
	for (const int64_t id: enemiesId)
	{
...
void CSideScrollScene::Start(float x, float y)
{
	...
	for (const std::pair<int64_t, CArea*>& p: areas)
	{
		const int64_t areaId = p.first;
		const CArea* area = p.second;
		float l, t, r, b;
...
void CSideScrollScene::SetArea(int64_t areaId)
{
...
void CSideScrollScene::UpdateEnemies(DWORD dt)
{
	...
	for (int64_t i = startY; i <= endY; i++)
	{
		for (int64_t j = startX; j <= endX; j++)
		{
...
{
	CPlayer* player = CPlayer::GetCurrentPlayer();
	int64_t grid_x = player->GetCenter().x / GRID_WIDTH;
	int64_t grid_y = player->GetCenter().y / GRID_HEIGHT;
...
{
	for (std::pair<int64_t, CAreaPortal*> por_pair: portals)
	{
		CAreaPortal* portal = por_pair.second;
		const int64_t id = por_pair.first;
		int64_t area1 = portal->GetAreaId();
		int64_t area2 = portal->GetId() % 10;
...
{
	for (const std::pair<int64_t, CAreaPortal*>& por_pair: portals)
	{
		CAreaPortal* portal = por_pair.second;
...
{
	std::list<LPSceneRequest> requests = CSceneRequest::RequestList;
...
void CSideScrollScene::RemoveEntity(int64_t id)
{
	LPEntity entity = mapEntities[id];

	if (entity == NULL) return;

	int64_t grid_x, grid_y;
...
void CSideScrollScene::SetEntity(LPEntity entity)
{
	...
	int64_t grid_x = entity->GetCenter().x / GRID_WIDTH;
	int64_t grid_y = entity->GetCenter().y / GRID_HEIGHT;

	//assert code
	if (grid_x < 0 || grid_y < 0 ||
		grid_x >= grid_count_width || grid_y >= grid_count_height)
	{
		DebugOut(L"[ERROR] std::vector size violation at side scroll scene ccp set entity\n");
		return;
	}
...
void CSideScrollScene::ResetEntityCoCollisionBoxes(LPEntity entity, int64_t grid_x, int64_t grid_y)
{
	std::list<LPCollisionBox> newCoCollisionBoxes;
	std::list<CSceneGrid*> selfAndSuroundGrids;
	...
	for (const CSceneGrid* grid: selfAndSuroundGrids)
	{
		for (const int64_t id: grid->GetColBoxes())
...
void CSideScrollScene::SetAreaTransition(CAreaPortal* p)
{
	...
	int64_t destAreaId = destPortal->GetAreaId();
...
void CSideScrollScene::DuringChangeAreaMode(DWORD dt)
{
	...
	int64_t sidex = (area_l - cam_l) > 0 ? 1 : -1;
	int64_t sidey = (area_t - cam_t) > 0 ? 1 : -1;
