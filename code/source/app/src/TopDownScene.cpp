...
void CTopDownScene::LoadPortals()
{
	...
	{
		int64_t id = ls[i][0];
		int64_t areaId = id / 100;
		int64_t reverseId = (id % 100) * 100 + (id / 100);

		int64_t l = ls[i][1] * 32 - 1;
		int64_t t = ls[i][2] * 32 - 1;

		int64_t r = ls[i][1] * 32 + 1;
		int64_t b = ls[i][2] * 32 + 1;

		int64_t sideX = ls[i][3];
		int64_t sideY = ls[i][4];
...
void CTopDownScene::Start(float x, float y)
{
	...
	for (std::pair<int64_t, CArea*> p: areas)
	{
		int64_t areaId = p.first;
		CArea* area = p.second;
		float l, t, r, b;
...
void CTopDownScene::SetArea(int64_t areaId)
{
...
void CTopDownScene::RemoveEntity(int64_t id)
{
...
void CTopDownScene::ResetEntityCoCollisionBoxes(LPEntity entity, int64_t grid_x, int64_t grid_y)
{
