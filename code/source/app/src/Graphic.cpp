...
bool IsFileExist(std::string i_filePath) {
	struct stat buffer;
	return (stat(i_filePath.c_str(), &buffer) == 0);
}

bool IsFileExist(std::wstring i_filePath) {
	std::string convertedString(i_filePath.begin(), i_filePath.end());
	return IsFileExist(convertedString);
}
