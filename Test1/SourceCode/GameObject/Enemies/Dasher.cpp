...
CDasher::CDasher(int _type): CEnemy()
{
	type = _type;
...
void CDasher::IdleState(DWORD dt)
{
	...
	if (type == DASHER_HORIZONTAL)
	{
		...
		int side = CPlayer::GetCurrentPlayer()->GetCenter().x - position.x > 0 ? 1 : -1;
		velocity.x = side * speed;
	}
	else
	{
		range = HorizontalDistanceToPlayer();
		int side = CPlayer::GetCurrentPlayer()->GetCenter().y - position.y > 0 ? 1 : -1;
